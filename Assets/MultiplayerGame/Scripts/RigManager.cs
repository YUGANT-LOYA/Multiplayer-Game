...
public class RigManager : MonoBehaviour
{
    ...
    [SerializeField] private Vector3 _weaponHandKickDirection = Vector3.zero;
    ...
    {
        set { _aimTarget.position = value; }
    }

    public float leftHandWeight
    {
        set { _leftHand.weight = value; }
    }
    ...
    public void SetLeftHandGripData(Vector3 pos, Vector3 rot)
    {
        if (_leftHand.data.target != null)
        {
            _leftHand.data.target.localPosition = pos;
            _leftHand.data.target.localEulerAngles = rot;
        }
    }
    ...
    private void Update()
    {
        ...
        {
            _rightHand.data.offset =
                Vector3.Lerp(_rightHand.data.offset, _originalRightHandOffset, 10f * Time.deltaTime);
        }

        if (_body.data.offset != _originalBodyOffset)
        {
            _body.data.offset =
                Vector3.Lerp(_body.data.offset, _originalBodyOffset, 10f * Time.deltaTime);
        }
    }
}